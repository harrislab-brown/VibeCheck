/*
 * vibecheck_accel.h
 *
 *  Created on: Sep 20, 2024
 *      Author: johnt
 */

#ifndef INC_VIBECHECK_ACCEL_H_
#define INC_VIBECHECK_ACCEL_H_

#include <stdint.h>
#include <math.h>
#include "stm32h7xx_hal.h"
#include "LSM6DS3.h"
#include "util.h"


/* interfaces with and manages data from the accelerometers */

#define VC_ACCEL_NUM_SENSORS 3
#define VC_ACCEL_DATA_PER_PACKET 10
#define VC_ACCEL_UPDATE_INTERVAL_MS 250

#define VC_ACCEL_DEFAULT_ODR 13
#define VC_ACCEL_DEFAULT_G_RANGE 2
#define VC_ACCEL_DEFAULT_DPS_RANGE 125

#define VC_ACCEL_MAX_OFFSET 0.1240234375f
static const uint32_t VC_ACCEL_ALLOWED_ODR[] = {13, 26, 52, 104, 208, 416, 833, 1660, 3330, 6660};
static const uint32_t VC_ACCEL_ALLOWED_G_RANGE[] = {2, 4, 8, 16};
static const uint32_t VC_ACCEL_ALLOWED_DPS_RANGE[] = {125, 245, 500, 1000, 2000};


typedef struct
{
	uint8_t id;
	uint32_t time;
	float x, y, z;
} VibeCheckAccel_Data;

typedef struct
{
	uint32_t is_connected;
	uint32_t accel_measuring;
	uint32_t gyro_measuring;
	volatile uint32_t received_data_flag;  /* flag set in interrupt when data received */
	uint32_t connection_change_flag;  /* flag set when the connection status was changed */
} VibeCheckAccel_SensorStatus;

typedef struct
{
	/* these will be zero-indexed which doesn't match the labels on the board - too bad */
	/*
	 * This code could perhaps be made cleaner by including the sensor status and
	 * configuration parameters directly in the LSM6DS3 struct.
	 */
	LSM6DS3_Config sensor_config[VC_ACCEL_NUM_SENSORS];
	LSM6DS3 sensor[VC_ACCEL_NUM_SENSORS];
	VibeCheckAccel_SensorStatus status[VC_ACCEL_NUM_SENSORS];

	/* combined sensor data buffer, and flags to handle double buffering */
	VibeCheckAccel_Data data[2 * VC_ACCEL_DATA_PER_PACKET];
	uint32_t data_ind;
	uint32_t data_ready;


	uint32_t time_prev_update;
	uint32_t generate_fake_data;  /* for testing */

} VibeCheckAccel;  /* TODO: should rename this because it's confusing since the sensors all have accelerometers and gyros */

void VibeCheckAccel_Init(VibeCheckAccel* accel, SPI_HandleTypeDef* hspi0, SPI_HandleTypeDef* hspi1, SPI_HandleTypeDef* hspi2);
void VibeCheckAccel_Update(VibeCheckAccel* accel);

/* interface functions */
void VibeCheckAccel_StartAccel(VibeCheckAccel* accel, uint32_t channel);  /* start acceleration measurement of specified channel */
void VibeCheckAccel_StopAccel(VibeCheckAccel* accel, uint32_t channel);

void VibeCheckAccel_StartGyro(VibeCheckAccel* accel, uint32_t channel);  /* start gyroscope measurement of specified channel */
void VibeCheckAccel_StopGyro(VibeCheckAccel* accel, uint32_t channel);

void VibeCheckAccel_SetAccelODR(VibeCheckAccel* accel, uint32_t channel, uint32_t odr);
void VibeCheckAccel_SetGyroODR(VibeCheckAccel* accel, uint32_t channel, uint32_t odr);
void VibeCheckAccel_SetAccelRange(VibeCheckAccel* accel, uint32_t channel, uint32_t range);
void VibeCheckAccel_SetGyroRange(VibeCheckAccel* accel, uint32_t channel, uint32_t range);
void VibeCheckAccel_SetOffsets(VibeCheckAccel* accel, uint32_t channel, float x, float y, float z);  /* accelerometer DC offsets in g */

uint32_t VibeCheckAccel_GetAccelODR(VibeCheckAccel* accel, uint32_t channel);
uint32_t VibeCheckAccel_GetGyroODR(VibeCheckAccel* accel, uint32_t channel);
uint32_t VibeCheckAccel_GetAccelRange(VibeCheckAccel* accel, uint32_t channel);
uint32_t VibeCheckAccel_GetGyroRange(VibeCheckAccel* accel, uint32_t channel);
void VibeCheckAccel_GetOffsets(VibeCheckAccel* accel, uint32_t channel, float* x, float* y, float* z);

void VibeCheckAccel_StartFakeData(VibeCheckAccel* accel);  /* for testing the host UI */
void VibeCheckAccel_StopFakeData(VibeCheckAccel* accel);

uint32_t VibeCheckAccel_ConnectionChanged(VibeCheckAccel* accel, uint32_t* channel, uint32_t* is_connected);

/* helper functions */
void VibeCheckAccel_UpdateSensor(VibeCheckAccel* accel, uint32_t channel);  /* send the new configuration parameters to a sensor chip */

/* data handling */
void VibeCheckAccel_AddData(VibeCheckAccel* accel, uint8_t id, uint32_t time, float x, float y, float z);
uint32_t VibeCheckAccel_GetDataReady(VibeCheckAccel* accel, VibeCheckAccel_Data** data);
void VibeCheckAccel_EXTICallback(VibeCheckAccel* accel, uint16_t GPIO_Pin);  /* call when the accelerometer data ready signal occurs */


#endif /* INC_VIBECHECK_ACCEL_H_ */
