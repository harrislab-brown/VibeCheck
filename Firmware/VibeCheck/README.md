...
Here is a MATLAB script for parsing the data from the VibeCheck. It is pretty slow but there’s probably a way to improve the speed with some MATLAB tricks. A good way to log the data is to use the VS Code serial monitor which can log everything from a monitoring session to a text file.