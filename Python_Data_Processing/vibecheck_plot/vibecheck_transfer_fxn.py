...
def select_port(ports: List[str]) -> str:
    """
    Prompt the user to select a serial port from a list of available ports.
    Args:
        ports (List[str]): A list of available serial port names.
    Returns:
current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
filename = f"data_{current_time}.csv"
current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
filename = f"data_{current_time}.csv"
        str: The selected serial port name.
    Raises:
        ValueError: If the user inputs an invalid choice.
    """
...
def input_with_timeout(prompt: str, timeout: float = 0.1) -> str:
    ...
    Returns:
        str: The user input if received, or an empty string if timed out.
    """
...
def main():
    """
    Main function to run the serial data logger.
    This function performs the following steps:
    1. Get available serial ports
    2. Prompt user to select a port
    3. Open the selected serial port
    4. Create a CSV file for logging data
    5. Continuously read data from the serial port and write to the CSV file
    6. Stop logging when the user presses Enter
    7. Close the serial port and exit
    """
    ...
    baud_rate = 9600  # You may need to adjust this based on your device
    try:
        ser = serial.Serial(selected_port, baud_rate)
        print(f"Connected to {selected_port}")
        vibecheck_init(ser)
        vibecheck_set_freq(ser, 30)
        current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"data_{current_time}.csv"
        with open(filename,'w', newline='') as csvfile:
            ...
            while True:
                ...
                if user_input:
                    print("Enter key pressed. Stopping data logging.")
                    ser.write(b'wavegen stop\n')
